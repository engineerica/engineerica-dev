...
let apiInstance = new AccuCampusOpenApi3.EventApi();
...
apiInstance.eventChecksessions(eventid, autorepair, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> eventListregistered(from, count, opts)
...
apiInstance.eventListregistered(from, count, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully.');
