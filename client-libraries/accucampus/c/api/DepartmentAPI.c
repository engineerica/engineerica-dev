...
DepartmentAPI_departmentDelete(apiClient_t *apiClient, char * id ,  body )
{
    ...
    // create the path
    long sizeOfPath = strlen("/department/{id}")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/department/{id}");
...
DepartmentAPI_departmentList(apiClient_t *apiClient, int from , int count )
{
    ...
    // create the path
    long sizeOfPath = strlen("/department/list")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/department/list");
