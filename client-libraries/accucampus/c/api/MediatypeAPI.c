...
MediatypeAPI_mediatypeDelete(apiClient_t *apiClient, char * id ,  body )
{
    ...
    // create the path
    long sizeOfPath = strlen("/mediatype/{id}")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/mediatype/{id}");
...
MediatypeAPI_mediatypeList(apiClient_t *apiClient, int from , int count )
{
    ...
    // create the path
    long sizeOfPath = strlen("/mediatype/list")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/mediatype/list");
