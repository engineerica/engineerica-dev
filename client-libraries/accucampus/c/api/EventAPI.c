...
EventAPI_eventChecksessions(apiClient_t *apiClient, char * eventid , int autorepair )
{
    ...
    // create the path
    long sizeOfPath = strlen("/event/checksessions")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/event/checksessions");
    ...
    // query parameters
    char *keyQuery_autorepair = NULL;
    int valueQuery_autorepair ;
    keyValuePair_t *keyPairQuery_autorepair = 0;
    if (autorepair)
    {
        keyQuery_autorepair = strdup("autorepair");
        valueQuery_autorepair = (autorepair);
        keyPairQuery_autorepair = keyValuePair_create(keyQuery_autorepair, &valueQuery_autorepair);
        list_addElement(localVarQueryParameters,keyPairQuery_autorepair);
    }
...
EventAPI_eventListregistered(apiClient_t *apiClient, char * from , char * count , int summaryonly , char * termid )
{
    ...
    // create the path
    long sizeOfPath = strlen("/event/listregistered")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/event/listregistered");
